export const techStack = [
  { name: "TypeScript", icon: "/icons/typescript.svg" },
  { name: "React", icon: "/icons/react.svg" },
  { name: "Redux", icon: "/icons/redux.svg" },
  { name: "Node.js", icon: "/icons/nodejs.svg" },
  { name: "Vite", icon: "/icons/vite.svg" },
  { name: "Vitest", icon: "/icons/vitest.svg" },
  { name: "Cypress", icon: "/icons/cypress.svg" },
  { name: "Playwright", icon: "/icons/playwright.svg" },
  { name: "Ruby on Rails", icon: "/icons/rails.png" },
  { name: "RSpec", icon: "/icons/rspec.png" },
  { name: "MongoDB", icon: "/icons/mongodb.svg" },
  { name: "date-fns", icon: "/icons/date-fns.svg", width: "100px" },
  { name: "AWS", icon: "/icons/aws.svg" },
  { name: "Kubernetes", icon: "/icons/kubernetes.svg" },
  { name: "jQuery", icon: "/icons/jquery.svg" },
  { name: "HTML", icon: "/icons/html5.svg" },
  { name: "CSS", icon: "/icons/css.png" },
  { name: "SASS", icon: "/icons/sass.svg" },
  { name: "Elasticsearch", icon: "/icons/elasticsearch.svg" },
  { name: "Redis", icon: "/icons/redis.svg" },
  { name: "RabbitMQ", icon: "/icons/rabbitmq.svg" },
  { name: "New Relic", icon: "/icons/newrelic.svg" },
  { name: "Sentry", icon: "/icons/sentry.svg" },
  { name: "HighCharts", icon: "/icons/highcharts.png" },
  { name: "React Flow", icon: "/icons/reactflow.svg" },
  { name: "ReactVis", icon: "/icons/reactvis.png" },
  { name: "BackboneJS", icon: "/icons/backbonejs.png" },
  { name: "JavaScript", icon: "/icons/javascript.svg" },
  { name: "NPM", icon: "/icons/npm.svg" },
  { name: "OAuth 2.0", icon: "/icons/oauth2.png" },
  { name: "Golang", icon: "/icons/golang.svg" },
  { name: "GitHub", icon: "/icons/github.svg" },
];
